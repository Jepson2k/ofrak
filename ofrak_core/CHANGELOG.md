...
- Add ESP PatchMaker support: `ESPAppProgramAttributesAnalyzer` (derives Xtensa/RISC-V `ProgramAttributes` from the chip) and `ESPAppAddSegmentModifier` (adds a loadable segment to create injectable free space), plus `InstructionSet.XTENSA`/`InstructionSet.RISCV`.
- Add `ESPAppExtendSegmentModifier`, which grows an existing loadable segment in place so a compiled patch can be injected into a flash-memory-mapped (IROM/DROM) region, where the ESP bootloader maps only one segment per region. Backed by an end-to-end test that injects a patch into a real ESP32 (Xtensa) and ESP32-C3 (RISC-V) image, redirects an existing call to it, repacks, and boots the image in Espressif QEMU to confirm the printed result changes.
